...
def generate_launch_description():
    ...
    # 1. RINOMINATO PER EVITARE IL CONFLITTO CON L'IIWA
    robot_state_publisher = Node(
        package="robot_state_publisher",
        executable="robot_state_publisher",
        name="bcr_bot_state_publisher", # <--- Modifica chiave!
        parameters=[
        ...
        remappings=[
            ('/joint_states', '/bcr_bot/joint_states'),           # Path assoluto
            ('/robot_description', '/bcr_bot/robot_description')  # Path assoluto
        ]
    )

    # 2. PATH ASSOLUTO PER IL TOPIC DI SPAWN
    gz_spawn_entity = Node(
        package="ros_gz_sim",
        executable="create",
        arguments=[
            "-topic", "/bcr_bot/robot_description", # <--- Modifica chiave!
            "-name", "bcr_bot",
            "-allow_renaming", "true",
            "-z", "0.28",
            "-x", position_x,
    ...
    # 3. PATH ASSOLUTI PER TUTTI I REMAPPING DEL BRIDGE
    gz_ros2_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name = "bcr_bot_ign_bridge",
        arguments=[
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("position_x", default_value="-1.5"),
